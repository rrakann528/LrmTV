...
-- muted_friends: old migration used 'muted_user_id' instead of 'friend_id'
DO $$ BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'muted_friends' AND column_name = 'muted_user_id'
  ) THEN
    ALTER TABLE muted_friends RENAME COLUMN muted_user_id TO friend_id;
  END IF;
END $$;

-- dm_read_receipts: old migration used 'other_user_id' instead of 'friend_id'
DO $$ BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'dm_read_receipts' AND column_name = 'other_user_id'
  ) THEN
